...
def drain_pty(master, f_log):
    """Read any remaining data from PTY after the main loop exits."""
    while True:
...
def run_analysis_on_device(cmd, log_file, cwd=None):
        ...
        try:
            while True:
                if time.time() - start_time > TIMEOUT_SECONDS:
                    print(f"\nTimeout: device did not reach POWER_MEASUREMENT_START within {TIMEOUT_SECONDS}s.")
                    break

                if p.poll() is not None:
                    drain_pty(master, f_log)
                    print("\nProcess exited before POWER_MEASUREMENT_START.")
                    break
    ...
    return success, current_ma, voltage_v
...
def parse_log_output(filename):
    ...
    with open(filename, 'r', errors='replace') as f:
        for line in f:
            ...
            # Accept both integer ms and float ms (e.g. "1234 ms" or "1.234 ms")
            if "Inference done in" in line:
                match = re.search(r'Inference done in ([0-9]+(?:\.[0-9]+)?)\s*ms', line)
                if match:
                    inf_times_ms.append(float(match.group(1)))

    avg_time = sum(inf_times_ms) / len(inf_times_ms) if inf_times_ms else 0
    return peaks, total_stack, avg_time, inf_times_ms
...
def plot_stack_usage(peaks, total_stack, output_file='stack_usage.png'):
    ...
    if total_stack == 0:
        print("WARNING: STACK_TOTAL not found in log. The stack limit line is an estimate (peak * 1.5).")
        print("         Emit 'STACK_TOTAL: <bytes>' from firmware for accurate headroom.")
        total_stack = int(peak_usage * 1.5)

    if peak_usage > total_stack:
        print(f"WARNING: peak usage ({peak_usage}) exceeds reported total stack ({total_stack}). "
              "Check your STACK_TOTAL firmware output.")
    ...
    ax1.axhline(y=total_stack / 1024, color='r', linestyle='--',
                label='Total Stack Limit' + (' (estimated)' if free_stack < 0 else ''))
    ax1.legend()
    ...
    ax2.text(0, peak_usage / 1024 / 2,
             f'{peak_usage / 1024:.1f} KB\n({peak_usage / total_stack * 100:.1f}%)',
             ha='center', va='center', color='white', fontweight='bold')
    ax2.text(0, (peak_usage + free_stack / 2) / 1024,
             f'Free: {free_stack / 1024:.1f} KB',
...
def main():
    ...
    peaks, total, avg_time_ms, all_times = parse_log_output(str(log_file))

    if not peaks:
        print("No STACK_PEAK entries found in log. Check firmware output.")
        sys.exit(1)

    peak = max(peaks)
    print(f"Stack peak:          {peak} bytes ({peak / 1024:.2f} KB)")
    if total:
        print(f"Stack total:         {total} bytes ({total / 1024:.2f} KB)")
        print(f"Headroom:            {total - peak} bytes ({(total - peak) / 1024:.2f} KB)")
    if all_times:
        print(f"Inference samples:   {len(all_times)}")
        print(f"Avg inference time:  {avg_time_ms:.2f} ms")
        print(f"Min / Max:           {min(all_times):.2f} ms / {max(all_times):.2f} ms")
    ...
    metrics = {
        ...
        'headroom_bytes': total - peak if total else None,
        'inference_samples': len(all_times),
        'avg_inference_time_ms': round(avg_time_ms, 3),
        'min_inference_time_ms': round(min(all_times), 3) if all_times else None,
        'max_inference_time_ms': round(max(all_times), 3) if all_times else None,
    }

    if current_ma is not None and voltage_v is not None:
        power_watts = (current_ma / 1000) * voltage_v
        power_mw = power_watts * 1000
        energy_joules = power_watts * (avg_time_ms / 1000)
        energy_mj = energy_joules * 1000

        print(f"\n--- Energy Estimates (at {voltage_v}V) ---")
        print(f"Active power draw:        {power_mw:.2f} mW")
        print(f"Energy per inference:     {energy_mj:.3f} mJ  ({energy_joules:.6f} J)")
        print("Note: these are board-level figures (USB bridge + LDO losses included).")

        metrics['active_current_ma'] = current_ma
        metrics['voltage_v'] = voltage_v
        metrics['active_power_mw'] = round(power_mw, 3)
        metrics['energy_per_inference_mj'] = round(energy_mj, 4)
        metrics['energy_per_inference_j'] = round(energy_joules, 7)
