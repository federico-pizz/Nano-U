...
MIN_IMAGES = 3
IMAGE_NAMES = ['test_img.bmp', 'test_img_1.bmp', 'test_img_2.bmp', 'test_img_3.bmp', 'test_img_4.bmp']
...
def plot_stack_usage(peaks, total_stack, output_file='stack_usage.png'):
    ...
    ax1.set_title('Stack Usage over Iterations (MobileNet Person Detect)', fontsize=12, fontweight='bold')
    ...
    ax2.set_title('Peak Stack Memory Usage (MobileNet Person Detect)', fontsize=12, fontweight='bold')
    ax2.legend()
...
def main():
    ...
    if len(found_images) < MIN_IMAGES:
        print(f"Found only {len(found_images)}/{MIN_IMAGES} required test images in {models_dir}:")
        for name in IMAGE_NAMES:
            status = "OK" if (models_dir / name).exists() else "MISSING"
            print(f"  [{status}] {name}")
        print(f"\nProvide at least {MIN_IMAGES} of the above BMP files to ensure benchmark diversity.")
        sys.exit(1)
    ...
    if peaks:
        ...
        if current_ma is not None:
            # Assumes measurement via USB cable from PC; change to 3.3 if measuring at 3V3 pin
            VOLTAGE = 5.0
