...
if __name__ == '__main__':
    ...
    DEVICE = "cpu"
    DATA_DIR = os.path.join(root_dir, "data/processed_data/test")
    ...
    bu_net.load_state_dict(torch.load(os.path.join(root_dir, "src/models/BU_Net.pth"), map_location=DEVICE))
    
    # Nano_U (using 2L version as float32)
    nano_u = Nano_U(n_channels=3)
    nano_u.load_state_dict(torch.load(os.path.join(root_dir, "src/models/Nano_U.pth"), map_location=DEVICE))
    
    # Nano_U_int8 (TFLite quantized)
    interpreter_int8 = tf.lite.Interpreter(model_path=os.path.join(root_dir, "src/models/Nano_U_int8.tflite"))
    ...
    models = {
        'BU_Net': {
            'model': bu_net, 
            'type': 'pytorch', 
            'path': os.path.join(root_dir, 'src/models/BU_Net.pth'), 
            'name': 'BU_Net'
        },
        'Nano_U': {
            'model': nano_u, 
            'type': 'pytorch', 
            'path': os.path.join(root_dir, 'src/models/Nano_U_2L.pth'), 
            'name': 'Nano_U'
        },
        'Nano_U_int8': {
            'model': interpreter_int8, 
            'type': 'tflite', 
            'path': os.path.join(root_dir, 'src/models/Nano_U_int8.tflite'), 
            'pytorch_params': 0, 
            'name': 'Nano_U_int8'
        }
    }
